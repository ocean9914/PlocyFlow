                        ...
                        foreach (var p in temp)
                        {
                            ...
                            if (policy != null && policy.Count()>0)
                            {
                                foreach (var pi in policy)
                                {
                                    ...
                                    b.Departmentid = CommonUtility.CommonUtility.TryParseStringToInt32(p.Element("DepartmentId").Value);
                                    b.ProductTypeid = p.Element("ProductTypeId").Value;
                                    b.OBDate = CommonUtility.CommonUtility.TryParseStringToDateTime(p.Element("OBDate").Value);
                                    b.GameTypeid = p.Element("GameTypeId").Value;
                                    b.Statusid = CommonUtility.CommonUtility.TryParseStringToInt32(p.Element("StatusId").Value);
